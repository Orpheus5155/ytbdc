...
async def skip(ctx: commands.Context, *args):
    ...
    if n_skips == 1:
        message = 'đang bỏ qua bài'
    elif n_skips < queue_length:
        message = f'đang bỏ qua `{n_skips}` trong `{queue_length}` số bài'
    else:
        message = 'đang bỏ qua tất cả bài'
...
async def sense_checks(ctx: commands.Context, voice_state=None) -> bool:
    ...
    if voice_state is None:
        await ctx.send('Bạn phải ở trong một kênh voice để dùng lệnh này')
        return False

    if bot.user.id not in [member.id for member in ctx.author.voice.channel.members] and ctx.guild.id in queues.keys():
        await ctx.send('Bạn phải ở trong một kênh voice cùng với bot để dùng lệnh này')
        return False
    return True
...
async def on_command_error(ctx: discord.ext.commands.Context, err: discord.ext.commands.CommandError):
    ...
    if isinstance(err, discord.ext.commands.errors.CommandNotFound):
        if BOT_REPORT_COMMAND_NOT_FOUND:
            await ctx.send("Lệnh đã gõ không hợp lệ. Để xem danh sách lệnh, hãy gõ: {}help".format(PREFIX))
        return
